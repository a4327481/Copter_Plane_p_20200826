...
% plots the selected data in the axes.
   ...
   set(f,'color','white')
    mPlotTypes      = {...      % Example plot types shown by this GUI
                    'Click Read Button',        @(a)plot(a, membrane)};
    data=ones(10,1);
    datacolumn=ones(10,1);
    Cnum=1;
    holdbutton=0;
    Cname{1,1}={'time'};
   %  Construct the components.
   hfftc = uicontrol('Style','pushbutton','String','FFT',...
          'Units','normalized',...
          'Position',[0.0,0.9,0.1,0.05],...
          'Callback',@FFTbutton_Callback);
   hsurf = uicontrol('Style','pushbutton','String','Read',...
          'Units','normalized',...  
          'Position',[0.9,0.9,0.1,0.05],...
          'Callback',@Readbutton_Callback);
   hmesh = uicontrol('Style','pushbutton','String','Plot',...
          'Units','normalized',...
          'Position',[0.4,0.9,0.1,0.05],...
          'Callback',@Plotbutton_Callback);
   hTaj = uicontrol('Style','pushbutton','String','Taj',...
          'Units','normalized',...
          'Position',[0.4,0.85,0.1,0.05],...
          'Callback',@PlotTaj_Callback);     
   hErrCode = uicontrol('Style','pushbutton','String','ErrCode',...
          'Units','normalized',...
          'Position',[0.55,0.85,0.1,0.05],...
          'Callback',@PlotErrCode_Callback);     
   hBaroAlt = uicontrol('Style','pushbutton','String','BaroAlt MR72',...
          'Units','normalized',...
          'Position',[0.7,0.85,0.1,0.05],...
          'Callback',@PlotBaroAlt_Callback);        
   hcontour = uicontrol('Style','pushbutton',...
          'String','Holdon',...
          'Units','normalized',...
          'Position',[0.55,0.9,0.1,0.05],...
          'Callback',@Holdonbutton_Callback); 
   hholdoff = uicontrol('Style','pushbutton',...
          'String','Holdoff',...
          'Units','normalized',...
          'Position',[0.7,0.9,0.1,0.05],...
          'Callback',@Holdoffbutton_Callback);    
%    htext = uicontrol('Style','text','String','Select Data',...
%           'Position',[325,90,60,15]);
    hPlotsPopupmenu=   uicontrol(...    % list of available types of plot
                    'Parent', f, ...
                    'Units','normalized',...
                    'Position',[0.11 0.85 0.25 0.1],...
...
%             current_data = peaks_data;AttChanged
...
function Readbutton_Callback(source,~) 
   % Display surf plot of the currently selected data.
       ...
       [filename, path, filterindex] = uigetfile('*.bin', '二进制文件 (*.bin)');
        if filename
      ...
      binname=strcat(path,caseFileName);
      fp=fopen(binname,'r');
        save binname binname;
...
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        algo_remote_ct_st_pitch=double(typecast(uint8(temp),'int16')')/32768*1; 
        temp=reshape([data(index_20,57:58)'],1,[]);
        algo_remote_ct_st_roll=double(typecast(uint8(temp),'int16')')/32768*1;           
        temp=reshape([data(index_20,59:60)'],1,[]);
        algo_remote_ct_st_throttle=double(typecast(uint8(temp),'int16')')/32768*1;           
        temp=reshape([data(index_20,61:62)'],1,[]);
        algo_remote_ct_st_yaw=double(typecast(uint8(temp),'int16')')/32768*1;           
...
%     data_um482
%%%%%%%%%%%%%%%%%
        mPlotTypes=data_imu_txt;%此处需要修改为需求的绘图项，如data_imu_txt data_baro_txt
data=[];
        for i=1:length(mPlotTypes)
        data(:,i)=eval(mPlotTypes{i,1});    
        end
   ...
   function Plotbutton_Callback(~,eventdata) 
   % Display mesh plot of the currently selected data.
        plot(data(:,1),data(:,hPlotsPopupmenu.Value),'LineWidth',2);
        grid(ha,'on');
        if(holdbutton==1)
            Cnum=Cnum+1;
        else
        ...
        Cname{Cnum,1}=mPlotTypes{hPlotsPopupmenu.Value,1};
        legend(Cname{1:Cnum,1});

   end
 
   function Holdonbutton_Callback(source,eventdata) 
   % Display contour plot of the currently selected data.
         hold(ha,'on');
         holdbutton=1; 
   end 
    function Holdoffbutton_Callback(source,eventdata) 
   % Display contour plot of the currently selected data.
         hold(ha,'off');
         holdbutton=0;
         Cnum=1;
    end 
    function FFTbutton_Callback(source,eventdata) 
   % Display contour plot of the currently selected data.
 
        Fs = 1/0.012;                    % Sampling frequency
        T = 1/Fs;                     % Sample time
   
                       % Time vector
        % Sum of a 50 Hz sinusoid and a 120 Hz sinusoid
        
        yy = data(round(50/0.012):round(300/0.012),hPlotsPopupmenu.Value);  
        L=length(yy);
        t = (0:L-1)*T; 
        yy=yy-mean(yy);
        NFFT = 2^nextpow2(L); 
        ...
        figure(199)
        plot(ff,2*abs(Y(1:NFFT/2+1)),'r') 
        grid on
        hold on
    end 

    function PlotTaj_Callback(source,~) 
   % Display contour plot of the currently selected data.
%         plot(data(:,1),data(:,hPlotsPopupmenu.Value));
%         grid(ha,'on');
%         if(holdbutton==1)
%             Cnum=Cnum+1;
%         else
...
%         Cname{Cnum,1}=mPlotTypes{hPlotsPopupmenu.Value,1};
%         legend(Cname{1:Cnum,1});
lmin=round(772/0.012);
lmax=round(777/0.012);
    figure(1000)
    plot(data(lmin:lmax,49),data(lmin:lmax,47),'k',data(lmin,49),data(lmin,47),'r*',data(lmax,49),data(lmax,47),'ro')
    axis equal
    grid on
    end 


    function PlotErrCode_Callback(source,~) 
     Count=binDecode(datacolumn,1,0,0);
      m=length(Count);
      time=[1:m]*0.012;

          tmp=binDecode(datacolumn,231,0,0);     
        tmp=reshape(tmp,4,m/4); 
        tmp1=[tmp(4,:);tmp(4,:);tmp(4,:);tmp(4,:)];
        code_num0_code0_L=reshape(tmp1,1,m)';   
        tmp1=[tmp(1,:);tmp(1,:);tmp(1,:);tmp(1,:)];
        code_num1_code0_L=reshape(tmp1,1,m)';
        tmp1=[tmp(2,:);tmp(2,:);tmp(2,:);tmp(2,:)];
        code_num2_code0_L=reshape(tmp1,1,m)';      
        
        tmp=binDecode(datacolumn,233,0,0);     
        tmp=reshape(tmp,4,m/4); 
        tmp1=[tmp(4,:);tmp(4,:);tmp(4,:);tmp(4,:)];
        code_num0_code0=reshape(tmp1,1,m)';%+code_num0_code0_L;   
        tmp1=[tmp(1,:);tmp(1,:);tmp(1,:);tmp(1,:)];
        code_num1_code0=reshape(tmp1,1,m)';%+code_num1_code0_L;
        tmp1=[tmp(2,:);tmp(2,:);tmp(2,:);tmp(2,:)];
        code_num2_code0=reshape(tmp1,1,m)';%+code_num2_code0_L;      
        

        tmp=binDecode(datacolumn,235,0,0);     
        tmp=reshape(tmp,4,m/4); 
        tmp1=[tmp(4,:);tmp(4,:);tmp(4,:);tmp(4,:)];
        code_num0_code1_L=reshape(tmp1,1,m)';   
        tmp1=[tmp(1,:);tmp(1,:);tmp(1,:);tmp(1,:)];
        code_num1_code1_L=reshape(tmp1,1,m)';
        tmp1=[tmp(2,:);tmp(2,:);tmp(2,:);tmp(2,:)];
        code_num2_code1_L=reshape(tmp1,1,m)';      
        
        tmp=binDecode(datacolumn,237,0,0);     
        tmp=reshape(tmp,4,m/4); 
        tmp1=[tmp(4,:);tmp(4,:);tmp(4,:);tmp(4,:)];
        code_num0_code1=reshape(tmp1,1,m)';%+code_num0_code1_L;   
        tmp1=[tmp(1,:);tmp(1,:);tmp(1,:);tmp(1,:)];
        code_num1_code1=reshape(tmp1,1,m)';%+code_num1_code1_L;
        tmp1=[tmp(2,:);tmp(2,:);tmp(2,:);tmp(2,:)];
        code_num2_code1=reshape(tmp1,1,m)';%+code_num2_code1_L; 
        
        figure(1001)
            plot(time,code_num0_code0_L,'r')
            title('Err from bit0 to bit15')
        figure(1002)
            plot(time,code_num0_code0,'r')  
            title('Err from bit16 to bit31')
        figure(1003)
            plot(time,code_num0_code1_L,'r')
            title('Err from bit32 to bit47')
        figure(1004)
            plot(time,code_num0_code1,'r') 
            title('Err from bit48 to bit63')
        figure(1005)
            plot(time,code_num1_code0_L,'r')
            title('Err from bit64 to bit79')
        figure(1006)
            plot(time,code_num1_code0,'r')  
            title('Err from bit80 to bit95')
        figure(1007)
            plot(time,code_num1_code1_L,'r')
            title('Err from bit96 to bit111')
        figure(1008)
            plot(time,code_num1_code1,'r') 
            title('Err from bit112 to bit127')
        figure(1009)
            plot(time,code_num2_code0_L,'r')
            title('Err from bit128 to bit143')
        figure(1010)
            plot(time,code_num2_code0,'r')     
            title('Err from bit144 to bit159')
        figure(1011)
            plot(time,code_num2_code1_L,'r')
            title('Err from bit160 to bit175')
        figure(1012)
            plot(time,code_num2_code1,'r')      
            title('Err from bit176 to bit191')          
        grid on
    end



    function PlotBaroAlt_Callback(source,~) 
        Count=binDecode(datacolumn,1,0,0);
        m=length(Count);
        time=[1:m]*0.012;

        tmp=binDecode(datacolumn,59,1,1); 
        tmp=reshape(tmp,2,m/2);           
        RY=[tmp(1,:);tmp(1,:)];
        RY=reshape(RY,1,m)'*2000.0;
      
        tmp=binDecode(data,165,1,1);
        tmp=reshape(tmp,8,m/8); 
        tmp1=[tmp(8,:);tmp(8,:);tmp(8,:);tmp(8,:);tmp(8,:);tmp(8,:);tmp(8,:);tmp(8,:)];
        MR72_Range01=reshape(tmp1,1,m)'*150.0;
        
        tmp=binDecode(data,205,1,1);
        tmp=reshape(tmp,8,m/8); 
        tmp1=[tmp(2,:);tmp(2,:);tmp(2,:);tmp(2,:);tmp(2,:);tmp(2,:);tmp(2,:);tmp(2,:)];
        BaroAlt1=reshape(tmp1,1,m)'*5000.0;
        
        tmp=binDecode(data,207,1,1);
        tmp=reshape(tmp,8,m/8); 
        tmp1=[tmp(2,:);tmp(2,:);tmp(2,:);tmp(2,:);tmp(2,:);tmp(2,:);tmp(2,:);tmp(2,:)];
        BaroAlt2=reshape(tmp1,1,m)'*5000.0;
        
        hold off;
        plot(time,RY,'LineWidth',2);
        hold on;
        plot(time,BaroAlt1,'LineWidth',2);
        plot(time,BaroAlt2,'LineWidth',2);
        plot(time,MR72_Range01,'LineWidth',2);
        legend('RY','BaroAlt1','BaroAlt2','MR72_Range01');
        hold off;
        grid on;
    end 
