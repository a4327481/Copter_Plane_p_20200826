...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
len_gps=length(data.GPS(:,7));
for i=2:len_gps
    if data.GPS(i,11)==237265
        data.GPS(i,11)=data.GPS(i-1,11);
        data.GPS(i,13)=data.GPS(i-1,13);
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% rz=1;
ry=[cosd(pitch_f(i))   0  sind(pitch_f(i)); 0              1               0;-sind(pitch_f(i)) 0 cosd(pitch_f(i))];
...
dQt=[wx_f(i) wy_f(i) wz_f(i)]*dt(i)/57.3*0;
...
% ,ts1,ax_f.*0.02,'r')
dVs=(dV(:,1).^2+dV(:,2).^2).^0.5;
Vx_f_gps=V_f_gps.*cosd(Gcrs_f);
Vy_f_gps=V_f_gps.*sind(Gcrs_f);
Vx_f_arsp=V_f_arsp.*cosd(Gcrs_f);
Vy_f_arsp=V_f_arsp.*sind(Gcrs_f);
nj=100;
for i=nj:len_dcm-nj
    V_f_gps_x_v(i+1,1)= sum(dV(i-nj+1:i,1));
%     V_f_gps_x_v(i+1,1)=Vx_f_arsp(i-nj+1)+sum(dV(i-nj+1:i,1));
...
i=1;
hold on
plot(ts1,Vx_f_arsp(:,1))
plot(ts1,V_f_gps_x_v(:,1))

plot(ts1(2:end),V_f_arsp(2:end)-V_f_arsp(1:end-1))
hold on
plot(t_arsp(2:end),V_arsp(2:end)-V_arsp(1:end-1))
plot(ts1,dVs)
% plot(ts1,Vy_f_arsp(:,1))

% plot( ts1,dV(:,1))
% plot(ts1(2:end),Vx_f_arsp(2:end)-Vx_f_arsp(1:end-1))

plot( ts1,dV(:,2))
plot(ts1(2:end),(Vy_f_arsp(2:end)-Vy_f_arsp(1:end-1)))
% hold on
% plot(ts1,Gcrs_f)
% plot(t_gps,Gcrs)

% ,ts1,V_f_gps_x_v(:,1),ts1,V_f_gps)
% plot(ts1,dV(:,1))
